...
#'
#' This function lists all datasets available in the 'DigestiveDataSets' package.
#' If the 'DigestiveDataSets' package is not loaded, it stops and shows an error message.
...
view_datasets_DigestiveDataSets <- function() {
  ...
  datasets_info <- utils::data(package = "DigestiveDataSets")$results

  # Check if there are datasets available
  if (nrow(datasets_info) == 0) {
    message("No datasets are currently available in the 'DigestiveDataSets' package.")
    return(character(0))
  }
  ...
  message("Datasets available in the 'DigestiveDataSets' package:")
